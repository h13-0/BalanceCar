...
MPU_Error_t MPU_GetAccelerometer(short *ax, short *ay, short *az)
{
	unsigned char tmp[6];

	if (!(st.chip_cfg.sensors & INV_XYZ_ACCEL))
		return MPU_AccelerometerNotEnabled;
...
MPU_Error_t MPU_GetTemperature(short *temperature)
{
	unsigned char tmp[2];
	short raw;

	if (!(st.chip_cfg.sensors))
	    return MPU_TemperatureSensorNotEnabled;
