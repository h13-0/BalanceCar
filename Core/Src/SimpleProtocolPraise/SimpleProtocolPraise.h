...
#define GeneratePraiseWithSuffixMethod(Byte, Suffix, SuffixLength, Buffer, MaximumBufferLength, DataLength, Callback) \
		{ \
			...
			{ \
				*(buffer + bufferIndex) = Byte; \
				...
				{ \
					if(!memcmp((buffer + bufferIndex - SuffixLength), Suffix, SuffixLength)) \
					{ \
						DataLength = bufferIndex; \
						Callback; \
						bufferIndex = 0; \
						memset(buffer, '\0', MaximumBufferLength); \
					} \
				} \
			} else { \
				bufferIndex = 0; \
...
#define PraiseWithPrefixAndSuffixAndStrictBufferLength(Byte, Prefix, PrefixLength, Suffix, SuffixLength, BufferLength)
